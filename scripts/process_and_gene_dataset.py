...
class Process_label_and_generate_dataset(object):
    ...
    def _xml2info(self, visual=False, box_offset=False, resize_size=(1920, 1080), visual_cls=False):
        ...
        with tqdm(total=len(os.listdir(self.imgpath))) as p_bar:
            ...
            for file in os.listdir(self.imgpath):
                # file = "03a98ad2d3a8aaa83eed08325351fe6a_2017-06-05.jpg"
                bbox_xywh = []
                ...
                if visual and visual_cls:
                    ...
                    cv2.imwrite(os.path.join(self.all_visual_path, name + '.jpg'), labelled, [int(cv2.IMWRITE_JPEG_QUALITY), 40])  # 0-100, 越小图片质量月底
                    ...
                    for i in range(len(bbox_xyxy)):
                        ...
                        # labelled = cv2.putText(labelled, self.classes[int(cls)], (int(xmax) + 2, int(ymax) + 2),
                        #                        cv2.FONT_HERSHEY_PLAIN,
                        #                        1, self.colors[int(cls)], 1)  # font scale, thickness
                        ...
                        #     cv2.imwrite(os.path.join(self.wdir, "subimg", str(cls), name + str(cls) + ".jpg"), subimg)
    ...
    def _read_xml_to_lst(self, image_id, plate_mode_type=0):
        ...
        for obj in root.iter('object'):
            ...
            # all type
            # qqcut
            # flag = cls.startswith("QQ-") or cls.startswith("QU-")   # for qu plate
            # if "/" in cls:  # for plate model1st
            #     cls = cls.split("/")[-1].split("_")[0].lower()
            #     if flag and cls == "green":
            #         cls = "qu"
            # if len(cls) < 3:
            #     continue
            ...
            # dangerous
            if self.project == "dangerous":
                if "sign_danger" not in cls:
                    continue
                cls = cls.split("sign_danger_")[-1]
                # merge 6.1(qt5) or 6.2(gr)to 2.3(yd)
                if cls == 'qt5' or cls == 'gr':
                    cls = 'yd'
                elif cls == 'qt1':
                    cls = 'yt1'
                elif cls == 'qt2':
                    cls = 'yt2'
            elif self.project == "plate":  # Thailand
                if len(cls) > 2:
                    cls = 'plate'
            elif self.project == "type" or self.project == "letter" or self.project == "digit":
                cls = cls.lower()
            else:
    ...
    def _enhance_img_xml(self, avg_nums, box_offset=False):
        ...
        """
        print("++++ enhance to max {} ++++".format(avg_nums))
    ...
    def _generate_dataset(self):
        ...
        if self.opt.label_box:
            ...
            if self.opt.flag_gray:
                ...
                img_train_path = os.path.join(self.wdir, "gray_dataset", 'images/train')
                label_train_path = os.path.join(self.wdir, "gray_dataset", 'labels/train')
                img_val_path = os.path.join(self.wdir, "gray_dataset", 'images/val')
                label_val_path = os.path.join(self.wdir, "gray_dataset", 'labels/val')
                img_test_path = os.path.join(self.wdir, "gray_dataset", 'images/test')
                label_test_path = os.path.join(self.wdir, "gray_dataset", 'labels/test')
            else:
        ...
        for set in self.sets:
            ...
            with tqdm(total=len(os.listdir(imgs_path))) as p_bar:
                ...
                for file in os.listdir(imgs_path):
                    ...
                    cv2.imwrite(os.path.join(retangele_img_path, name + '.jpg'), labelled, [int(cv2.IMWRITE_JPEG_QUALITY), 20])
                    p_bar.update(1)
        ...
        if mode == 1:  # for plate
            ...
            with tqdm(total=len(files)) as p_bar:
                for xmlFile in files:
                    ...
                    for obj in root.iter('object'):  # 获取object节点中的name子节点
                        ...
                        plate_type = tmp_name.lower().split("/")[-1]
                        if "/" in tmp_name:
            ...
            with tqdm(total=len(files)) as p_bar:
                for xmlFile in files:
                    ...
                    root = dom.getroot()  # 获取根节点
                    for obj in root.iter('object'):  # 获取object节点中的name子节点
                        tmp_name = obj.find('name').text.lower()  # 找到object中 name 的具体名称
                        # tmp_name = tmp_name
    ...
    def _count_subdir(self, wdir):
        """
        :param wdir: 需要统计的文件夹路径
        :return:
        """
        subfolder_count = 0  # 子文件夹数量计数器
    ...
    def run(self):
        ...
        not_exists_path_make_dirs([self.infopath, self.all_visual_path, self.error_img_path,
                                   self.error_xml_path, self.lstpath, self.enhance_path])
        # filter_different_suffix_with_the_same_name_of_2dirs(self.imgpath, self.xmlpath, os.path.join(self.wdir, "error"))
        ...
        if self.opt.xml2info:
            self._xml2info(visual=self.opt.all_visual, box_offset=self.opt.label_box,
                           resize_size=self.opt.resize_size, visual_cls=self.opt.visual_cls)  # mode_type: default=0, 1 for plate
        ...
        if self.opt.enhance and os.path.exists(self.enhance_path):
            ...
            self._enhance_img_xml(150, box_offset=self.opt.label_box)  # max_nums-1 don't for the cls of max_nums
        ...
        if self.opt.visualization:
            if self._count_subdir(os.path.join(self.wdir, "images")) == 3 and \
               self._count_subdir(os.path.join(self.wdir, "labels")) == 3:
        ...
        if self.opt.diff:
            print("filter xml ...")
            filter_different_suffix_with_the_same_name_of_2dirs(self.infopath, self.xmlpath, os.path.join(self.wdir, "error/xml"), ".txt")  # 文件少的后缀
            print("filter img ...")
            filter_different_suffix_with_the_same_name_of_2dirs(self.infopath, self.imgpath, os.path.join(self.wdir, "error/img"), ".txt")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='data preprocess')
    parser.add_argument('--project_type', type=str, default="type", help='detection type, for xml2info')
    parser.add_argument('--wdir', type=str, default="/cv/all_training_data/container/type/dataset/",
    ...
    parser.add_argument('--visual_cls', type=bool, default=False, help="visual for all dataset")
    parser.add_argument('--count_cls', type=bool, default=True, help="count the number of class")
