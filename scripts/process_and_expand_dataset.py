...
from file_operate import Count_the_number_of_directory, save_file_according_suffixes, read_txt_and_return_list
...
class Process_label_and_expand_dataset:
    def __init__(self, args):
        self.opt = args

    def _init_path(self):
        self.source_path = self.opt.source_path
        self.imgpath = os.path.join(self.source_path, 'img')  # img path
        self.xmlpath = os.path.join(self.source_path, 'xml')  # xml path
        self.sets = ['train', 'val', 'test']

        # source path
        if self.opt.label_box:
            self.dataset = os.path.join(self.source_path, str(self.opt.resize_size[0]) + "x" + str(self.opt.resize_size[1]))
            ...
            self.infopath = os.path.join(self.source_path, 'info')  # xml 2 info: [cls, x, y, w, h]
            self.all_visual_path = os.path.join(self.source_path, 'all_visualization')  # 保存画框后图片的路径
            self.error_img_path = os.path.join(os.path.join(self.source_path, "error/img"))
            self.error_xml_path = os.path.join(os.path.join(self.source_path, "error/xml"))
            self.lstpath = os.path.join(self.source_path, 'ImageSets')
            self.enhance_path = os.path.join(self.source_path, "enhance")
            # not_exists_path_make_dirs([self.infopath, self.all_visual_path, self.error_img_path,
            #                            self.error_xml_path, self.lstpath, self.enhance_path])

        # target path
        self.target_path = self.opt.target_path
        self.target_imgpath = os.path.join(self.target_path, 'img')  # target img path
        self.target_xmlpath = os.path.join(self.target_path, 'xml')  # target xml path

        self.target_infopath = os.path.join(self.target_path, 'info')  # xml 2 info: [cls, x, y, w, h]
        self.target_all_visual_path = os.path.join(self.target_path, 'all_visualization')  # 保存画框后图片的路径
        self.target_error_img_path = os.path.join(os.path.join(self.target_path, "error/img"))
        self.target_error_xml_path = os.path.join(os.path.join(self.target_path, "error/xml"))
        self.target_lstpath = os.path.join(self.target_path, 'ImageSets')
        self.target_enhance_path = os.path.join(self.target_path, "enhance")
        not_exists_path_make_dirs([self.target_infopath, self.target_all_visual_path, self.target_error_img_path,
                                   self.target_error_xml_path, self.target_lstpath, self.target_enhance_path])

    def _load_labelmap(self):
        # load labelmap
        jsonhander = JsonHandler(os.path.join(self.opt.source_path, "labelmap.json"))
        ...
        with tqdm(total=len(os.listdir(self.imgpath))) as p_bar:
            ...
            for file in os.listdir(self.imgpath):
                ...
                if visual and visual_cls:
                    visual_cls_path = os.path.join(os.path.join(self.source_path, "visualizaition_cls"))
    ...
    def _read_xml_to_lst(self, image_id, plate_mode_type=0):
        ...
        for obj in root.iter('object'):
            ...
            if cls == "yeiiow" or cls == "hellow" or cls == "yeiiow":
                print(cls, image_id)
                cls = "yellow"
            elif cls == "greem" or cls == "green]":
                cls = "qu"
    ...
    def _enhance_img_xml(self, avg_nums, box_offset=False):
        ...
        if not box_offset:
            enhance_output_img_path = os.path.join(self.source_path, 'img')
            # imgpath = os.path.join(source_path, 'img')
        else:
    ...
    def _generate_train_val_test_lst(self):
        ...
        # infopath = os.path.join(source_path, 'info')   # img path
        # txtsavepath = os.path.join(source_path, 'ImageSets')
    ...
    def _generate_dataset(self):
        ...
        if self.opt.label_box:
            ...
            if self.opt.flag_gray:
                imgpath = os.path.join(self.source_path, 'gray')
            else:
                imgpath = os.path.join(self.source_path, 'img')

            # output path for train, val
            img_train_path = os.path.join(self.source_path, 'images/train')
            label_train_path = os.path.join(self.source_path, 'labels/train')
            img_val_path = os.path.join(self.source_path, 'images/val')
            label_val_path = os.path.join(self.source_path, 'labels/val')
            img_test_path = os.path.join(self.source_path, 'images/test')
            label_test_path = os.path.join(self.source_path, 'labels/test')
    ...
    def _show_label_and_count_class_frequence(self, mode=0):
        ...
        _dict = dict()
        _dict["label"] = classes
        jsonhander = JsonHandler(os.path.join(self.source_path, "labelmap.json"))
        jsonhander.save_json(_dict)

        with open(os.path.join(self.source_path, "origin_labelmap_fre.txt"), 'w') as fn:
    ...
    def run(self):
        ...
        #         save_file_according_suffixes(self.source_path)
        #
        # not_exists_path_make_dirs([self.infopath, self.all_visual_path, self.error_img_path,
        #                            self.error_xml_path, self.lstpath, self.enhance_path])
        #
        # # 2. show label
        # if not os.path.exists(os.path.join(self.source_path, "labelmap.json")):
        ...
        # # 1. split
        # if self.opt.split:
        #     split_img_labelfile_to_distpath(self.source_path, self.source_path, ".xml")
        #
        # # 2. show label
        # if self.opt.show_label_save:
        #     self._show_label_and_count_class_frequence(mode=self.opt.data_mode)  # 1 for plat
        # if os.path.exists(os.path.join(self.source_path, "labelmap.json")):
        #     self._load_labelmap()
        ...
        #     rgb2gray_for_imgset_and_save(self.imgpath, os.path.join(self.source_path, "gray"))
        #
        # generate dataset
        ...
        # if self.opt.diff:
        #     print("filter xml ...")
        #     filter_different_suffix_with_the_same_name_of_2dirs(self.infopath, self.xmlpath, os.path.join(self.source_path, "error/xml"), ".txt")  # 文件少的后缀
        #     print("filter img ...")
        #     filter_different_suffix_with_the_same_name_of_2dirs(self.infopath, self.imgpath, os.path.join(self.source_path, "error/img"), ".txt")


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='data preprocess')
    parser.add_argument('--source_path', type=str, default="/cv/xyc/Datasets/taiguo_yancan/labeled/0428/检测/front",
                        help="Source dataset directory")
    parser.add_argument('--target_path', type=str, default="/cv/all_training_data/yancan/xinhaida/det/front/",
    ...
    parser.add_argument('--xml2info', type=bool, default=True,
    ...
    parser.add_argument('--src2target', type=bool, default=True, help="move source path to target")
    args = parser.parse_args()

    process_label_and_expand_dataset = Process_label_and_expand_dataset(args)
    process_label_and_expand_dataset.run()
