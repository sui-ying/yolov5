...
from scripts_for_image import diff_img_lablefile_and_move_extra
from tools import not_exists_path_make_dir
...
def xml2info(imgpath, xmlpath, mode_type=0):
    ...
    :return:
    """
    with tqdm(total=len(os.listdir(xmlpath))) as p_bar:
        p_bar.set_description('xml2info')
        for file in os.listdir(xmlpath):

            if file[:-4] + '.jpg' in os.listdir(imgpath):
                imgname = os.path.join(imgpath, file[:-4] + '.jpg')
                # h, w, c = cv2.imread(imgname).shape
                parse_info = read_xml_to_lst(xmlpath, file[:-4], mode_type)  # [cls, x, y, w, h]
            elif file[:-4] + '.png' in os.listdir(imgpath):
                imgname = os.path.join(imgpath, file[:-4] + '.png')
                parse_info = read_xml_to_lst(xmlpath, file[:-4], mode_type)  # [cls, x, y, w, h]
            else:
                continue
            ...
            out_file = open(os.path.join(wdir, 'info', file[:-4] + '.txt'), 'w')
...
def read_xml_to_lst(xmlpath, image_id, plate_mode_type):
    """
    read object info
    name -> cls
    ...
    if plate_mode_type == 0:    # for all data, except plate
        for obj in root.iter('object'):
            ...
            # for container
            if cls == "zero" or cls == "o":
                cls = "zero_O"
            elif cls == "one" or cls == "i":
                cls = "one_I"
            elif cls == "two" or cls == "z":
                cls = "two_Z"
            elif cls == "five" or cls == "s":
                cls = "five_s"
            elif cls == "eight" or cls == "b":
                cls = "eight_B"
            else:
                cls = cls

            # # lorry
            # if cls == "tray-vertical":
            #     cls = "tray"
            # elif cls == "container-vertical":
            #     cls = "container-vertical"
            # elif cls == "truckhead-down":
            #     cls = "truckfront"
            # elif cls == 'lorry-down' or cls == 'lorry-up':
            #     cls = "lorry"
            # elif cls == "truckhead-up":
            #     cls = "truckback"
            # else:
            #     continue
...
def count_class_frequency(wdir, infopath, cls_nums, flag_plot=True):
    for i in range(len(classes)):
...
def enhance_img_xml(wdir, imgpath, avg_nums):
    ...
    for enhance_cls in os.listdir(os.path.join(wdir, 'enhance')):
        if os.path.isdir(os.path.join(wdir, 'enhance', enhance_cls)):
            total_times = int(
                avg_nums / len(os.listdir(os.path.join(wdir, 'enhance', enhance_cls)))) - 1   # 400 -> average number of cls
            if total_times >= 10:
                total_times = 10
            else:
            ...
            for i in range(total_times):
                ...
                with tqdm(total=len(os.listdir(os.path.join(wdir, 'enhance', enhance_cls)))) as p_bar:
                    ...
                    for info_file in os.listdir(os.path.join(wdir, 'enhance', enhance_cls)):  # the least class
                        ...
                        # save image
                        enhance_image.save(os.path.join(enhance_output_img_path, 'enh_' + str(counter) + '_' + name + '.jpg'))
                        # save info
...
def img_info_2_images_labels(wdir):
...
def parse_args():
    ...
    parser.add_argument('--wdir', type=str, default="/cv/xyc/Datasets/container/mix3", help="Dataset directory")
...
if __name__ == '__main__':
    ...
    # view = ""
    # if view == "aerial":
    #     classes = ['ao', 'crack', 'hole']
    #     wdir = "/media/westwell/dataset/nansha4/det/huangshujiao/labeled/nansha4_aerial_20220830"
    # elif view == "front":
    #     classes = ['ao', 'crack', 'hole']  # front
    #     wdir = "/cv/xyc/nansha/det/frontback/dataset"
    # elif view == "top":
    #     classes = ['hole', 'silver']
    #     wdir = "/media/westwell/dataset/nansha4/det/huangshujiao/labeled/top"
    # else:
    #     classes = ['ao', 'crack', 'hole', 'lockhole']
    #     wdir = "/media/westwell/dataset/nansha4/det/huangshujiao/labeled/nansha4_side_20220830"

    # classes = ['blue_plate', 'green_plate', 'white_plate', 'yellow_plate']
    # classes = ['a', 'ah', 'b', 'bj', 'c', 'cq', 'd', 'e', 'eight', 'f', 'five', 'fj', 'four', 'g', 'gd', 'gs', 'gx', 'gz', 'h', 'ha', 'hb', 'he', 'hi', 'hl', 'hn', 'j', 'jl', 'js', 'jx', 'k', 'l', 'ln', 'm', 'n', 'nine', 'nm', 'nx', 'one', 'p', 'q', 'qh', 'r', 's', 'sc', 'sd', 'seven', 'sh', 'six', 'sn', 'sx', 't', 'three', 'tj', 'two', 'u', 'v', 'w', 'x', 'xj', 'xz', 'y', 'yn', 'z', 'zero', 'zj']
    # classes = ['wan', 'podong/l', 'podong/s']
    # wdir = "/cv/xyc/yolov5/plate/dataset_1st"
    wdir = opt.wdir
    sets = ['train', 'val', 'test']
    ...
    if opt.count_cls:   # count classes nums
        ...
        count_class_frequency(wdir, infopath, cls_nums)
    ...
    if opt.enhance and os.path.exists(os.path.join(wdir, 'enhance')):
        ...
        enhance_img_xml(wdir, imgpath, 600-1)  # max_nums-1 don't for the cls of max_nums

    # 4 make txt
    if opt.make_lst:
        generate_train_val_test_lst(wdir)

    # 5 generate dataset
    if opt.gene_dataset:
        img_info_2_images_labels(wdir)
