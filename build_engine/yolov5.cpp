...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir, save_dir)) {
        ...
        std::cerr << "./yolov5 -d [.engine] ../samples save_dir // deserialize plan file and run inference" << std::endl;
        return -1;
    }

    if (!save_dir.empty()) {
        // 0777表示文件夹的权限，该权限和使用chmod改变文件/文件夹权限类似
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        doInference(*context, stream, buffers, data, output, BATCH_SIZE);
        auto end = std::chrono::system_clock::now();
        std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        std::vector<std::vector<Yolo::Detection>> batch_res(fcount);
        for (int b = 0; b < fcount; b++) {
            auto& res = batch_res[b];
            nms(res, &output[b * OUTPUT_SIZE], CONF_THRESH, NMS_THRESH);
        }
        for (int b = 0; b < fcount; b++) {
            auto& res = batch_res[b];
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(0xFF, 0xFF, 0xFF), 1.5);
            }
